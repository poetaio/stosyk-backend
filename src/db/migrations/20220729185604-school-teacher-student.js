            ...
            queryInterface.createTable(...SchoolTeacherTable, {transaction})
                ...
                .then((teacherIds) => Promise.all(
                    teacherIds[0].map((teacherId) => {
                        const id = v4();
                        return queryInterface.sequelize.query(`INSERT INTO schools("schoolId", name)
                                                               values ('${id}', 'default name')`, {transaction})
                            .then(() =>
                                queryInterface.sequelize.query(`INSERT INTO "schoolTeachers"("schoolId", "teacherId")
                                                                VALUES ('${id}', '${teacherId}')`, {transaction}))
                    })
                ))
